...
const BookingSection = ({ selectedService = '', selectedPackage = '', onServiceSelect, onPackageSelect }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'date' && value) {
      if (!isValidDate(value)) {
        ...
        // Don't clear the field, just show it as invalid
        return;
      }
    }
  ...
  // Auto-select first valid date when field is interacted with
  ...
  // Handle various interaction events to ensure mobile compatibility
  ...
  // Handle click on date container - enhanced for mobile
  const handleDateContainerClick = () => {
    // Auto-select before any other interaction
    autoSelectDate();
    
    // Small delay to ensure the value is set before picker opens
    setTimeout(() => {
  ...
  return (
    <section id="booking" className="py-20 bg-gray-50 dark:bg-gray-800 transition-colors duration-300">
                    ...
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (e.target.value && !isValidDate(e.target.value)) {
                          ...
                          // Don't clear invalid dates, just show them as invalid
                        }
                      }}
                      className={`w-full px-4 py-3 rounded-lg border ${errors.date ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'} bg-white dark:bg-gray-600 text-gray-900 dark:text-white focus:border-blue-500 dark:focus:border-blue-400 focus:ring focus:ring-blue-200 dark:focus:ring-blue-800 focus:ring-opacity-50 appearance-none`}
                    />
                  </div>
                  {errors.date && (
      ...
      {showPackageInfo && (
        ...
        >
          <div 
            className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md sm:max-w-lg lg:max-w-2xl w-full max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <div className="p-4 lg:p-6 space-y-4 lg:space-y-6">
              {formData.service && (
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 lg:p-4 mb-4">
                  <p className="text-sm lg:text-base text-blue-800 dark:text-blue-200">
              ...
              <div className="grid gap-4 lg:gap-6">
                {getAvailablePackages(formData.service).find(pkg => pkg.id === 'basic') ? (
                  ...
                  <div className="border rounded-lg p-4 lg:p-5 bg-gray-100 dark:bg-gray-800 opacity-60 border-gray-300 dark:border-gray-600">
                    <h4 className="font-semibold text-gray-600 dark:text-gray-400 mb-2 lg:mb-3 text-lg">Basic Package - $199 (Not Available)</h4>
                    <p className="text-sm lg:text-base text-gray-500 dark:text-gray-400 mb-2 lg:mb-3">
                      Not available for {formData.service.split('-').map(word => 
                        word.charAt(0).toUpperCase() + word.slice(1)
                      ).join(' ')} services due to complexity requirements.
                    </p>
                    <ul className="text-sm lg:text-base text-gray-500 dark:text-gray-400 space-y-1 lg:space-y-2">
